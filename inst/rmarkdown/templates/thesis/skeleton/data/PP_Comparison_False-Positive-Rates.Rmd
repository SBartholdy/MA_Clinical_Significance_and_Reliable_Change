---
title: "PP: Comparison of False-Positive Rates"
...
PP_5.5 = read.delim("cor_07_k62/cor_07_dataset_k62.txt", row.names=NULL) %>% 
...
PP_5.5$Diff_1.1 = PP_5.5$PRE0_1 - PP_5.5$POST0_5
...
## Ausschluss von Fällen mit Intervall-SDs == 0: NICHT AUSGEFÜHRT

```{r eval=FALSE}
#length(which(PP_5.5$ind.pretestSD == 0)) #93
#length(which(PP_5.5$ind.posttestSD == 0)) #97

PP_5.5 = PP_5.5 %>% 
  filter(ind.pretestSD != 0 & ind.posttestSD != 0)

PP_5.5$ID = 1:nrow(PP_5.5)
```
...
save(PP_5.5_ts, file = "Time Series Dataframes/k62_PP_5.5_ts.RData")

###

load("Time Series Dataframes/k62_PP_5.5_ts.RData")
...
ggsave("Time Series Dataframes/k62_PP_5.5_Pre-Post_Box_Violin.jpg", plot = Pre_Post_Box_Violin, width = 6, height = 4)
save(Pre_Post_Box_Violin, file = "Time Series Dataframes/k62_PP_5.5_Pre_Post_Box_Violin.RData")
...
ggsave("Time Series Dataframes/k62_PP_5.5_Pre-Post_Box_Violin_Mean+CI.jpg", plot = Pre_Post_Box_Violin_Mean_CI, width = 6, height = 4)
save(Pre_Post_Box_Violin_Mean_CI, file = "Time Series Dataframes/k62_PP_5.5_Pre_Post_Box_Violin_Mean_CI.RData")
```

```{r fig.align="center", out.width="75%"}
#knitr::include_graphics("Time Series Dataframes/k62_PP_5.5_Pre-Post_Box_Violin.jpg")
knitr::include_graphics("Time Series Dataframes/k62_PP_5.5_Pre-Post_Box_Violin_Mean+CI.jpg")
```

```{r message=FALSE, warning=FALSE}
print(dfSummary(PP_5.5[,12:17], varnumbers = FALSE, plain.ascii = FALSE, style = 'grid', graph.magnif = 0.75, 
...
temp = tibble(Score_Diff = c(PP_5.5$MeanDiff, PP_5.5$Diff_1.1), 
              Type = rep(c("MeanDiff", "Diff_1.1"), each = length(PP_5.5$MeanDiff)))
...
scatter.hist(PP_5.5$Diff_1.1, PP_5.5$MeanDiff, xlab = "PP_5.5$Diff_1.1 (blue)",
             ylab = "PP_5.5$MeanDiff (orange)", ellipse = FALSE, grid = TRUE, col = c("dodgerblue", "darkorange"))
```
...
summary(PP_5.5$Cohen_d)

(cohen_d_5.5 = (mean(PP_5.5$PRE_Mean) - mean(PP_5.5$POST_Mean)) / 
    sqrt(0.5 * (mean(PP_5.5$ind.pretestSD)^2 + mean(PP_5.5$ind.posttestSD)^2)))
```
...
PP_5.5$PC = (1 - (PP_5.5$POST0_1 / PP_5.5$PRE0_1)) * 100
...
```{r}
PP_5.5$RCI_JT = (PP_5.5$POST0_1 - PP_5.5$PRE0_1) / sqrt(2 * (sd(PP_5.5$PRE0_1) * sqrt(1 - PP_5.5_Alpha)) ^ 2)
...
> z.B. testen mit which(is.nan(PP_5.5$RCI_ind_preSD_klass))
...
```{r}
PP_5.5$EN_5.5_min = (PP_5.5_Alpha * (PP_5.5$PRE_Mean - mean(PP_5.5$PRE_Mean)) + 
                       mean(PP_5.5$PRE_Mean)) - 2 * mean(PP_5.5$ind.pretestSD) * sqrt(1 - PP_5.5_Alpha)
PP_5.5$EN_5.5_max = (PP_5.5_Alpha * (PP_5.5$PRE_Mean - mean(PP_5.5$PRE_Mean)) + 
                       mean(PP_5.5$PRE_Mean)) + 2 * mean(PP_5.5$ind.pretestSD) * sqrt(1 - PP_5.5_Alpha)
...
PP_5.5$EN_1.1_min = (PP_5.5_Alpha * (PP_5.5$PRE0_1 - mean(PP_5.5$PRE0_1)) + 
                       mean(PP_5.5$PRE0_1)) - 2 * sd(PP_5.5$PRE0_1) * sqrt(1 - PP_5.5_Alpha)
PP_5.5$EN_1.1_max = (PP_5.5_Alpha * (PP_5.5$PRE0_1 - mean(PP_5.5$PRE0_1)) + 
                       mean(PP_5.5$PRE0_1)) + 2 * sd(PP_5.5$PRE0_1) * sqrt(1 - PP_5.5_Alpha)
...
PP_5.5 = PP_5.5 %>% 
  mutate(CSI_1.1_klass = case_when(
    PRE0_1 >= 10 & POST0_1 <= 9 & PC >= 50 ~ -1,
    PRE0_1 <= 9 & POST0_1 >= 10 & PC <= -50 ~ 1,
    TRUE ~ 0))
```
...
> Übereinstimmung von PHQ-Interpretationen (je pre und post) hinzufügen ???
